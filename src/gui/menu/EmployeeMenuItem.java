...
public class EmployeeMenuItem extends JPanel {
	...
	private EmployeePopupSubmenu popup;

	public EmployeeMenuItem(EmployeeMenu menu, String menus[], int menuIndex, List<MenuEvent> events) {
		this.menu = menu;
		this.menus = menus;
	...
	private void init() {
		...
		for (int i = 0; i < menus.length; i++) {
			...
			if (i == 0) {
				...
				menuItem.addActionListener((ActionEvent e) -> {
					if (menus.length > 1) {
						if (menu.isMenuFull()) {
							ManagerMenuAnimation.animate(EmployeeMenuItem.this, !menuShow);
						} else {
							popup.show(EmployeeMenuItem.this, (int) EmployeeMenuItem.this.getWidth() + UIScale.scale(5),
		...
		popup = new EmployeePopupSubmenu(getComponentOrientation(), menu, menuIndex, menus);
	}
